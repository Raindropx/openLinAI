...
- 当前维护分支：`main`。
- v1.2.0 发布基线：`5d1b65a`；远端标签 `v1.2.0` 与 `v1.2.0-public` 均指向该提交。
- 远端：`origin/main` 在本次验收开始时与发布基线一致；后续状态必须通过实时 `git fetch` 重新确认。
- 最近工作重点：响应式工作台、模板与任务管理、角色卡库、图片生成元数据；此前已完成图片生成引擎和 Nano Banana 图片比例兼容。
