...
interface ImageCropModalProps {
  ...
  onConfirmCopy: (croppedDataUrl: string) => Promise<void>
}
...
export function ImageCropModal({
  ...
  const handleSubmit = async (action: SubmitAction) => {
    ...
    try {
      ...
      await (action === 'copy' ? onConfirmCopy : onConfirm)(croppedDataUrl)
    } catch (error) {
      ...
      footer={
        <div className="flex flex-wrap justify-between gap-2">
          ...
          <div className="flex flex-wrap justify-end gap-2">
            <Button
              disabled={submitting || !crop || !!loadError}
              onClick={handleReset}
            >
              还原
            </Button>
            <Button
              loading={submittingAction === 'copy'}
              disabled={submitting || !crop || !!loadError}
            ...
            <Button
              type="primary"
              loading={submittingAction === 'replace'}
              disabled={submitting || !crop || !!loadError}
